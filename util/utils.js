...
// Mimics iconv-lite basic syntax
const encoder = {
    // string to buffer
...
function encodingWrapper(encoding, iconv) {
    this.encoder = iconv || encoder;

    if (!encoding || !this.encoder.encodingExists(encoding)) {
        this.encoding = "utf8";
    } else {
...
encodingWrapper.toString = function (buf) {
    if (Buffer.isBuffer(buf)) return this.encoder.decode(buf, this.encoding);
    return typeof buf === "string" ? buf : String(buf);
};

encodingWrapper.toBuffer = function (str) {
    if (Buffer.isBuffer(str)) return str;
    return this.encoder.encode(String(str), this.encoding);
};
